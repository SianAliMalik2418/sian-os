...
import { CalendarDays, Droplets, Dumbbell, RefreshCw, Scale } from 'lucide-react'
...
function WeeklyReviewPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-5 px-3 py-5 sm:space-y-6 sm:px-6 sm:py-7 lg:px-10 lg:py-10">
    ...
    <Card><CardHeader><CardTitle>Review history</CardTitle><CardDescription>Permanent weekly snapshots</CardDescription><CardAction><CalendarDays className="size-5 text-primary" /></CardAction></CardHeader><CardPanel className="space-y-3">{reviews.length ? reviews.map((review) => <button type="button" key={String(review.id)} onClick={() => setWeekStart(String(review.week_start))} className="flex w-full flex-col gap-2 rounded-xl border p-4 text-left transition hover:bg-accent sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">Week of {String(review.week_start)}</p><p className="mt-1 text-sm text-muted-foreground">{String(review.wins || review.focus_next_week || 'Review saved')}</p></div><div className="flex gap-2"><Badge variant="outline">{String(review.workouts_completed)} workouts</Badge></div></button>) : <p className="py-10 text-center text-sm text-muted-foreground">Build your first weekly review above.</p>}</CardPanel></Card>
  </div>
}
