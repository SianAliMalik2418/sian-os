...
import { CalendarDays, Droplets, RefreshCw, Scale } from 'lucide-react'
...
function WeeklyReviewPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { week_start: weekStart, wins: String(form.get('wins') || ''), lessons: String(form.get('lessons') || ''), focus_next_week: String(form.get('focus_next_week') || '') }
    try {
  ...
  return <div className="mx-auto max-w-7xl space-y-5 px-3 py-5 sm:space-y-6 sm:px-6 sm:py-7 lg:px-10 lg:py-10">
    ...
    <section className="grid gap-3 sm:grid-cols-3 sm:gap-4">
      ...
      <Stat icon={CalendarDays} label="Nutrition consistency" value={current?.nutrition_consistency ? `${current.nutrition_consistency}/10` : '—'} />
    ...
    <Card><CardHeader><CardTitle>Review history</CardTitle><CardDescription>Permanent weekly snapshots</CardDescription><CardAction><CalendarDays className="size-5 text-primary" /></CardAction></CardHeader><CardPanel className="space-y-3">{reviews.length ? reviews.map((review) => <button type="button" key={String(review.id)} onClick={() => setWeekStart(String(review.week_start))} className="flex w-full flex-col gap-2 rounded-xl border p-4 text-left transition hover:bg-accent sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">Week of {String(review.week_start)}</p><p className="mt-1 text-sm text-muted-foreground">{String(review.wins || review.focus_next_week || 'Review saved')}</p></div><Badge variant="outline">{review.water_consistency ? `${review.water_consistency}/10 water` : 'Saved'}</Badge></button>) : <p className="py-10 text-center text-sm text-muted-foreground">Build your first weekly review above.</p>}</CardPanel></Card>
  </div>
}

function Stat({ icon: Icon, label, value }: { icon: typeof Scale; label: string; value: string }) { return <Card><CardHeader><Icon className="mb-2 size-5 text-primary" /><CardDescription>{label}</CardDescription><CardTitle className="text-2xl tabular-nums">{value}</CardTitle></CardHeader></Card> }
