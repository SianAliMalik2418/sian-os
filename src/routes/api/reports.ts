  ...
  server: {
    handlers: {
      GET: async ({ request }) => handleApi(async () => {
        ...
        const checkins = await db().prepare('SELECT date, weight_kg, sleep_hours, water_liters, protein_grams, fat_grams, carb_grams, calories FROM daily_checkins WHERE date BETWEEN ? AND ? ORDER BY date').bind(lowerBound, upperBound).all<DailyCheckin>()
