    ...
    handlers: {
      ...
      POST: async ({ request }) => handleApi(async () => {
        ...
        const result = await db().prepare(`
          INSERT INTO daily_checkins (date, weight_kg, sleep_time, wake_time, sleep_hours, water_liters, protein_grams, calories, nutrition_notes, workout_text, notes, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
          ON CONFLICT(date) DO UPDATE SET weight_kg=excluded.weight_kg, sleep_time=excluded.sleep_time, wake_time=excluded.wake_time, sleep_hours=excluded.sleep_hours, water_liters=excluded.water_liters, protein_grams=excluded.protein_grams, calories=excluded.calories, nutrition_notes=excluded.nutrition_notes, workout_text=excluded.workout_text, notes=excluded.notes, updated_at=CURRENT_TIMESTAMP
          RETURNING *
        `).bind(input.date, nullable(input.weight_kg), nullable(input.sleep_time), nullable(input.wake_time), nullable(sleepHours), nullable(input.water_liters), nullable(input.protein_grams), nullable(input.calories), nullable(input.nutrition_notes), nullable(input.workout_text), nullable(input.notes)).first<DailyCheckin>()
