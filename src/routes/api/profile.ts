  ...
  server: {
    handlers: {
      ...
      PUT: async ({ request }) => handleApi(async () => {
        ...
        await db().prepare(`
          INSERT INTO profile (id, height_cm, weight_kg, age, goals, experience_level, training_style, gym_schedule, equipment, injuries, long_term_vision, updated_at)
          ...
          ON CONFLICT(id) DO UPDATE SET height_cm=excluded.height_cm, weight_kg=excluded.weight_kg, age=excluded.age, goals=excluded.goals,
            ...
            equipment=excluded.equipment, injuries=excluded.injuries, long_term_vision=excluded.long_term_vision, updated_at=CURRENT_TIMESTAMP
        `).bind(nullable(input.height_cm), nullable(input.weight_kg), nullable(input.age), nullable(input.goals), nullable(input.experience_level), nullable(input.training_style), nullable(input.gym_schedule), nullable(input.equipment), nullable(input.injuries), nullable(input.long_term_vision)).run()
