...
import { readRecipe, recipeFromForm, recipeId } from '@/lib/recipes'
  ...
  server: {
    handlers: {
      PUT: async ({ request, params }) => handleApi(async () => {
        ...
        const parsed = await recipeFromForm(await request.formData(), existing)
        try {
          const result = await db().prepare(`
            UPDATE recipes
            SET name = ?, aliases = ?, category = ?, serving_description = ?, calories = ?, protein_grams = ?, ingredients = ?, notes = ?, photo_r2_key = ?, photo_content_type = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
            RETURNING *
          `).bind(parsed.values.name, parsed.values.aliases, parsed.values.category, parsed.values.serving_description, parsed.values.calories, parsed.values.protein_grams, parsed.values.ingredients, parsed.values.notes, parsed.values.photo_r2_key, parsed.values.photo_content_type, id).first<Recipe>()
          if (parsed.oldPhotoKey) await env.FILES.delete(parsed.oldPhotoKey)
          await recordApiWrite('update', 'recipe', id, { name: parsed.values.name, calories: parsed.values.calories, protein_grams: parsed.values.protein_grams })
