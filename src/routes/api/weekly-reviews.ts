  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => handleApi(async () => {
        ...
        const database = db()
        const [workouts, weights, nutrition, bestWorkout] = await Promise.all([
        ...
        await database.prepare(`
          INSERT INTO weekly_reviews (week_start, workouts_completed, missed_workouts, body_weight_change, nutrition_consistency, water_consistency, best_workout, wins, lessons, focus_next_week)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          ON CONFLICT(week_start) DO UPDATE SET workouts_completed=excluded.workouts_completed, missed_workouts=excluded.missed_workouts,
            ...
            best_workout=excluded.best_workout, wins=excluded.wins, lessons=excluded.lessons, focus_next_week=excluded.focus_next_week
        `).bind(
          input.week_start,
          workoutsCompleted,
