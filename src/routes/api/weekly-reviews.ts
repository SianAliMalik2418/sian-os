    ...
    handlers: {
      ...
      POST: async ({ request }) => handleApi(async () => {
        ...
        await database.prepare(`
          INSERT INTO weekly_reviews (week_start, body_weight_change, nutrition_consistency, water_consistency, wins, lessons, focus_next_week)
          VALUES (?, ?, ?, ?, ?, ?, ?)
          ON CONFLICT(week_start) DO UPDATE SET body_weight_change=excluded.body_weight_change,
            nutrition_consistency=excluded.nutrition_consistency, water_consistency=excluded.water_consistency,
            wins=excluded.wins, lessons=excluded.lessons, focus_next_week=excluded.focus_next_week
        `).bind(
          input.week_start,
          weightChange,
